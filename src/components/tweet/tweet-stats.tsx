...
import { TweetBuySellToggle } from './tweet-buy-sell-toggle';
import type { Tweet } from '@lib/types/tweet';

type TweetStatsProps = Pick<
  Tweet,
  'userLikes' | 'userRetweets' | 'userReplies' | 'userBuys' | 'userSells' 
> & {
  reply?: boolean;
  userId: string;
  isOwner: boolean;
...
export function TweetStats({
  ...
  const totalBuys = userBuys?.length;
  const totalSells = userSells?.length;
  ...
  const tweetIsBuy = userBuys?.includes(userId);
  const tweetIsSell = userSells?.includes(userId);
    ...
    <>
      ...
      <div>
        <TweetBuySellToggle 
          userId={userId} 
          tweetId={tweetId} 
          tweetIsBuy={tweetIsBuy} 
          tweetIsSell={tweetIsSell} 
          totalBuys={totalBuys} 
